...
export class BillSummaryComponent {
    ...
    calculateBillsByCategory(): Array<{ amount: number, label: string}> {
        ...
        for (let categoryTotalId in this.billsByCategory) {
            if (this.billsByCategory.hasOwnProperty(categoryTotalId)) {
            billsAverageByCategory.push(
                {
                    amount: this.billsByCategory[categoryTotalId] / monthDelta, label: this.getCategory(parseInt(categoryTotalId, 10))
                });
            }
        }
